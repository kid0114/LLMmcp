...
LLMmcp 是一个简单、清晰、可扩展的 MCP 项目骨架。当前已实现十一个可运行 MCP Server：`search`、`fetch`、`browser`、`local_file`、`github_search`、`time`、`file_reader`、`video`、`paper`、`modelscope`、`huggingface`。
...
`fetch://{encoded_url}` resource template 作为客户端误走 resources 时的
兼容兜底。
...
如需读取你自己有权限访问的 Medium 文章，可在本地 `.env` 设置 `MEDIUM_COOKIE`；不要把真实 cookie 写入 `mcp.example.json` 或提交到仓库。
复制浏览器 Network 里的 Medium `cookie` 后，可用 `make set-medium-cookie` 从剪贴板更新 `.env`。
...
访问 Medium 页面时同样会读取本地 `MEDIUM_COOKIE` 并注入浏览器上下文。
