...
- `modelscope MCP`：查询 ModelScope 上公开可索引的 skill、dataset、model、paper 趋势资源
- `huggingface MCP`：查询 Hugging Face 上公开可索引的 paper、model、dataset 趋势资源
- 提供统一配置、权限检查、日志和基础测试
...
- `resource_type` 当前支持 `skill`、`dataset`、`model`、`paper`
...
- `resource_type` 当前支持 `paper`、`model`、`dataset`
- `period` 支持 `today`、`week`、`10d`、`14d`、`month` 以及通用 `Nd`
